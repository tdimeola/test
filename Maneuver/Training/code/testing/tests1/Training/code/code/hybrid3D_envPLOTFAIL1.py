...
#                i=1
#                while i<4:
                    for cell in list_of_elems:
                        cell1 = float(cell)
                        cell2 = float(cell)
                        cell3 = float(cell)
                # Add contents of list as last row in the csv file
                 csv_writer.writerow(cell1, cell2, cell3)

        append_list_as_row('trajectory_position.csv', self.rigid_body.position)
#        with open('trajectory_position.csv', mode='w') as traj_file:
#            traj_file_writer = csv.writer(traj_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
#            traj_file_writer.writerow(self.rigid_body.position)

#        plt.plot(self.rigid_body.position)
