...
class Hybrid3DEnv(gym.Env):
 ...
 #                       boxposList = random.sample(range(-5, 5), 3)
                        try:
                            data = pandas.read_csv('trajectory_4.csv', names=colnames)
                            print('data in csv is: ', data)
                            time.sleep(6)
                        except:
                            append_list_as_row('trajectory_4.csv', colnames)
                            data = pandas.read_csv('trajectory_4.csv', names=colnames)             
                            print('colnames are x, y, z? ', data)
                            time.sleep(6)
...
#                        z = z+1
                        boxposList = [x,y,z]
#                        print('boxposList is:', x,y,z)
   
#                        boxposString = ' '.join(map(str, boxposList))
#                        print('boxposString is:', boxposString) 
                        subelem.set('pos', boxposList)
