...
class Hybrid3DEnv(gym.Env):
 ...
 #       self.box_init = list(0,0,4)
 #       self.set_box_positions(data_folder + config_file,box_init)
        ...
        self.rigid_body2 = rigid_body.RigidBody(mass = self.mass, inertia_body = self.inertia_tensor)
        self.state = None
    ...
    def parse_xml_tree(self, root):
        ...
        if root.tag == "render_file":
            ...
            print('render_filename =', self.render_filename) 
...
#        print('now = np.hstack((vel, rpy[2]))', now)
#        time.sleep(3)
    ...
    def update_state(self, config_file):
        # get local vel
        vel_local = self.calc_local_velocity(self.rigid_body.rpy[2], self.rigid_body.velocity)
        vel_local = self.calc_local_velocity(self.rigid_body2.rpy[2], self.rigid_body2.velocity)
        now_state = np.concatenate([self.rigid_body.position, self.rigid_body.rpy, vel_local, self.rigid_body.omega_body])
        now_state = np.concatenate([self.rigid_body2.position, self.rigid_body.rpy, vel_local, self.rigid_body.omega_body])


        append_list_as_row('trajectory_4.csv', self.rigid_body.position)
        append_list_as_row('trajectory_4.csv', self.rigid_body2.position)
 #       set_box_positions(data_folder + config_file, self.rigid_body.position)
    # set box positions
        print('config_file is:', config_file)
        xml_tree = ET.parse(config_file)
        root = xml_tree.getroot()
        
        for render_file in root.findall('render_file'):
            filename = render_file.find('filename')
            filename = render_file.get('filename')   
        ...
        for elem in root:
            ...
            while (i<6 ):
                for subelem in elem.findall('body'):
                    if subelem.get('name') == 'box'+ str(i):
                        boxpos = subelem.get('pos')

                #Generate 3 random numbers between 0 and 30 and make them into a space separated string
                        boxposList = random.sample(range(-5, 5), 3)   
                        boxposString = ' '.join(map(str, boxposList)) 

#                for elem in root:
#                     for subelem in elem.findall('body'):
#                        if subelem.get('name') == 'box'+ 'i':
                        subelem.set('pos', boxposString)
                        print('pos is ', boxposString)
                        time.sleep(3)
                        print('box name is: ', subelem.get('name'))
                        print('i = ', i)
                i=i+1

            tree.write("../data/" + filename)
...
########################Fix Hardcoded Name - TD ###########################

        self.update_state('../data/' + 'QuadPlanex.xml')
    ...
    def step(self, action):
        ...
        self.rigid_body2.advance(self.dt)
...
################Fix hardcoding TD##############################################
        self.update_state('../data/' + 'QuadPlane.xml')
        ob = self.get_observation_vector()
...
#	done = done or 
        ...
        if self.play and self.iter % self.render_intervel == 0:
            ...
            qpos[3] = self.rigid_body2.position[1]
            qpos[4] = self.rigid_body2.position[0]
            qpos[5] = - self.rigid_body2.position[2]
            ...
            qpos[6] = VIS_Orientation[0]
            qpos[7] = VIS_Orientation[1]
            qpos[8] = VIS_Orientation[2]
            qpos[9] = VIS_Orientation[3]
